...
function openReading() {
  ...
  setText('reading-theme', data.theme);
  setText('reading-title', reading.title);
  setText('reading-source', reading.source);
  setHtml('reading-body', reading.body);

  // Personal reflection varies by user
  const user = AppState.currentUser;
  const reflection = reading.reflection || {};
  const ref = reflection[user] || reflection.shared || 'Take three slow breaths, then begin reading.';
  setText('reading-reflection', ref);
